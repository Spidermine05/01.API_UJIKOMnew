...
<body class="bg-light">
    <nav class="navbar navbar-expand-lg navbar-dark bg-primary mb-4">
        <div class="container">
            <a class="navbar-band" href="#"> Panel Peminjam</a>
            <div class="d-flex">
                        ...
                        @forelse($alats as $index => $alat)
                            <tr>
                                <td class="text-center">
                                    <input type="checkbox" nanme="alat_id[]" value="{{ $alat->id }}" class="form-check-input">
                                </td>
                                <td>{{ $alat->nama_alat }}</td>
                                <td>{{ $alat->katgeori->nama_kategori }}</td>
                                <td>{{ $alat->stok }}</td>
                                <td>
                                    <input type="number" nanme="jumlah[]" class="form-control form-control-sm" value="1" min="1" max="{{ $alat->stok }}">                                   
                                </td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="5" class="text-cneter">Tidak ada yang tersedia saat ini.</td>                            
                            </tr>
                        @endforelse
                    </tbody>
                </table>
